...
#pragma omp parallel for schedule(static, 1) num_threads(ltp->jobs_load)
    for (int i = 0; i < count; i++) {
...
#else // _WIN32
                    std::cout << "loading stbi_image..." << std::endl;
        ...
        if (pixeldata) {
            ...
            if (c == 4 &&
                (ext == PATHSTR("jpg") || ext == PATHSTR("JPG") || ext == PATHSTR("jpeg") || ext == PATHSTR("JPEG"))) {
...
#else
            success = stbi_write_png(v.outpath.c_str(), v.outimage.w, v.outimage.h, v.outimage.elempack,
                                     v.outimage.data, 0);
#endif
...
#else
            success = stbi_write_jpg(v.outpath.c_str(), v.outimage.w, v.outimage.h, v.outimage.elempack,
                                     v.outimage.data, 100);
#endif
        }
        if (success) {
...
#endif
{
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (filename_noext == last_filename_noext) {
                    ...
                    fprintf(stderr, "both %s and %s output %s ! %s will output %s\n", filename.c_str(),
                            last_filename.c_str(), output_filename.c_str(), filename.c_str(), output_filename2.c_str());
#endif
    ...
    std::cout << "finding the model on disk..." << std::endl;
    int prepadding = 0;
    ...
    std::cout << "starting the waifu2x routine..." << std::endl;
...

    {
        ...
        // main routine
        {
            ...
            std::cout << "calling the convoluted load.." << std::endl;
